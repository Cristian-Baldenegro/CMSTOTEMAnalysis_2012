...
DataAnalysis::DataAnalysis (void){
	cout << "File with histos from data is created" << endl;
}
...
void DataAnalysis::getHistos (bool unc_jec_up, bool unc_jec_dw, bool unc_pf_up, bool unc_pf_dw, bool unc_rp_y_up, bool unc_rp_y_dw, bool unc_rp_x, bool unc_xi_up, 
	...
	TFile* data_file;
    if (unc_jec_up) data_file = TFile::Open("~/cernbox/doctorado/note/scripts/root_files/rereco/data_ntuple_jet_up_jecwinter.root","READ");
    else if (unc_jec_dw) data_file = TFile::Open("~/cernbox/doctorado/note/scripts/root_files/rereco/data_ntuple_jet_dw_jecwinter.root","READ");
    else if (unc_pf_up) data_file = TFile::Open("~/cernbox/doctorado/note/scripts/root_files/rereco/data_ntuple_pf_up_jecwinter.root","READ");
    else if (unc_pf_dw) data_file = TFile::Open("~/cernbox/doctorado/note/scripts/root_files/rereco/data_ntuple_pf_dw_jecwinter.root","READ");
    ...
    TH1F_histos["xi_right_cut_sasha"] = new TH1F("xi_right_cut_sasha","",11,xi_bins);
    ...
    for(int i_evt = 0; i_evt < nev_data; ++i_evt){
        ...
        if (jet_sel && proton_right_kin_sel && valid_proton_right_data && rp_right_data){
            ...
            if (xi_right_totem_bh>0 && xi_right_totem_bh<0.1 &&  t_right_totem_bh>0.03 && t_right_totem_bh<1 && xi_right_cms_bh - xi_right_totem_bh<0) TH1F_histos["t_right_cut_bh"]->Fill(t_right_totem_bh, 1);
        ...
        if (jet_sel && proton_right_kin_sel && valid_proton_right_data){
            if (rp_right_top_data && fid_cut_right_top) TH1F_histos["xi_cms_minus_totem_right_top"]->Fill(xi_cms_minus_data - xi_proton_right_data, 1.);
            if (rp_right_bottom_data && fid_cut_right_bottom) TH1F_histos["xi_cms_minus_totem_right_bottom"]->Fill(xi_cms_minus_data - xi_proton_right_data, 1.);
        ...
        if (jet_sel && proton_left_kin_sel && valid_proton_left_data && rp_left_data){
            ...
            if (xi_left_totem_bh>0 && xi_left_totem_bh<0.1 &&  t_left_totem_bh>0.03 && t_left_totem_bh<1 && xi_left_cms_bh - xi_left_totem_bh<0) TH1F_histos["t_left_cut_bh"]->Fill(t_left_totem_bh, 1);
        ...
        if (jet_sel && proton_left_kin_sel && valid_proton_left_data){
            if (rp_left_top_data && fid_cut_left_top) TH1F_histos["xi_cms_minus_totem_left_top"]->Fill(xi_cms_plus_data - xi_proton_left_data, 1.);
            if (rp_left_bottom_data && fid_cut_left_bottom) TH1F_histos["xi_cms_minus_totem_left_bottom"]->Fill(xi_cms_plus_data - xi_proton_left_data, 1.);
...
void DataAnalysis::AssymError (TH1F* histo_unfolded_nominal, TH1F* histo_jes_up_unfolded, TH1F* histo_jes_dw_unfolded, TH1F* histo_pf_up_unfolded, 
    ...
    for(int i = initial; i <= Nbins; ++i){
        ...
        y_up[i] = y[i]*sqrt(pow(jes_up[i]-1, 2));// + pow(pf_up[i]-1, 2) + pow(xi_up[i]-1, 2) + pow(fabs(backg[i])/2, 2) + pow(fabs(rew_beta[i])/2, 2) +  
              // pow(fabs(rp[i])/2, 2) + pow(fabs(gauss[i])/2, 2) + pow(fabs(sector[i])/2, 2) + pow(fabs(b_slope[i])/2, 2));// + 
...
int main() {
    ...
    DataAnalysis data2;
    // data2.getHistos(false,false,false,false,false,false,false,true,false);
}