...
double getScaleFactor(double etaJet, int variation){
    ...
    while(ifile.is_open() && !ifile.eof()) { 
	 ifile >> etaMin >> etaMax >> val >> sf >> sf_dw >> sf_up; 
         ...
         if (etaJet<=etaMax && etaJet >etaMin){
             if (variation == 0) scaleFactor = sf;
             else if (variation == 1) scaleFactor = sf_up;
             else if (variation == -1) scaleFactor = sf_dw;
             else {std::cout << "Possible variation values: 1, -1 and 0" << std::endl; exit(1);}
         }
    } 
    ifile.close(); 
