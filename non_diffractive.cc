...
void non_diffractive(string const& mc = "pythia6", string const& pt_range = "QCD_pt_15_3000", const Int_t nevt_max = -1){
  ...
  TString outputFileName = "/storage/lhuertas/uerj-1/CMSTOTEM/mc/Workspace/root_files/" + mc + "_" + pt_range + ".root";
...
//  float xi_bins_2[9] = {-0.05, 0, 0.028, 0.048, 0.065, 0.08, 0.1, 0.2};
  float xi_bins_2[8] = {-0.05, 0, 0.028, 0.048, 0.065, 0.08, 0.1, 0.2};
  //float_t bin_sasha[4] ={0.0003, 0.002, 0.0045, 0.01};
...
//      histosTH1F["hltTrigFired"]->GetXaxis()->SetBinLabel( (k + 1) , hltPathNames[k].c_str() );
  ...
  histosTH1F["xi_correction"] = new TH1F("xi_correction", "xi_correction" , 50, 0, 2);
  histosTH1F["xi_cms_rec"] = new TH1F("xi_cms_rec", "xi_proton_minus" , 7, xi_bins_2);
  histosTH1F["xi_cms_rec_bin"] = new TH1F("xi_cms_rec_bin", "xi_proton_minus" , 8, bin_sasha);
  histosTH1F["xi_cms_gen"] = new TH1F("xi_cms_gen", "xi_proton_minus" , 7, xi_bins_2);
  histosTH1F["xi_cms_gen_bin"] = new TH1F("xi_cms_gen_bin", "xi_proton_minus" , 8, bin_sasha);
  ...
  histosTH1F["xi_proton_signal_right_kinxi"] = new TH1F("xi_proton_signal_right_kinxi", "xi_proton_right" , 8, bin_sasha);
  ...
  histosTH1F["xi_cms_gen_pt20"] = new TH1F("xi_cms_gen_pt20", "xi_cms" , 8, bin_sasha);
  histosTH1F["xi_proton_backg_right_kinxi"] = new TH1F("xi_proton_backg_right_kinxi", "xi_proton_right" , 8, bin_sasha);
...
//         if( ( (fabs(eta) <= 2.866) && (fabs(eta) > 3.152) ) || (fabs(eta) <= 4.730) ){
	 ...
	 if (status == 1) {
      ...
      histosTH1F["xi_correction"]->Fill(correction, event_weight);
  ...
  //    MyBaseJet const& secondJet = ( pfJet_coll->at(1) ).mapjet["ak5PFJets"];
      ...
      if (selectJet_gen){
          ...
          histosTH1F["xi_cms_gen"]->Fill(xi_minus_gen, event_weight);
          histosTH1F["xi_cms_gen_bin"]->Fill(xi_minus_gen, event_weight);
          if ( !log10(x_minus) && log10(x_minus_gen) )response_xminus.Miss (log10(x_minus_gen), event_weight);
          if ( !log10(x_plus) && log10(x_plus_gen) ) response_xplus.Miss (log10(x_plus_gen), event_weight);
      }
      if (selectJet20_gen) histosTH1F["xi_cms_gen_pt20"]->Fill( xi_minus_gen , event_weight );
      ...
      if (select_Vertex && selectJet_rec){
         ...
         histosTH1F["xi_cms_rec"]->Fill(xi_minus_Reco, event_weight);
         histosTH1F["xi_cms_rec_bin"]->Fill(xi_minus_Reco, event_weight);
         if ( log10(x_minus) && !log10(x_minus_gen) ) response_xminus.Fake (log10(x_minus), event_weight);
         if ( log10(x_plus) && !log10(x_plus_gen) ) response_xplus.Fake (log10(x_plus), event_weight);

         if (selectJet_gen){
            response_xminus.Fill (log10(x_minus), log10(x_minus_gen), event_weight);
            response_xplus.Fill (log10(x_plus), log10(x_plus_gen), event_weight);
      ...
      if (select_Vertex && selectJet_rec && signal_right){
          ...
          histosTH1F["xi_proton_signal_right_kinxi"]->Fill( xi_proton_minus_rec , event_weight );
      ...
      if (select_Vertex && selectJet_rec && backg_right){
         ...
         histosTH1F["xi_proton_backg_right_kinxi"]->Fill( -xi_proton_right_zb , event_weight );
...
/*  TH1F* logx_minus_data = (TH1F*)data->Get("log_x_minus_jj");
  TH1F* logx_plus_data = (TH1F*)data->Get("log_x_plus_jj");

  RooUnfoldBayes x_jj_minus_unfold_bayes (&response_xminus, logx_minus_data, 4);
  RooUnfoldBayes x_jj_plus_unfold_bayes (&response_xplus, logx_plus_data, 4);
  TH1F* x_jj_minus_data_unfolded_bayes = (TH1F*) x_jj_minus_unfold_bayes.Hreco();
  TH1F* x_jj_plus_data_unfolded_bayes = (TH1F*) x_jj_plus_unfold_bayes.Hreco();
  x_jj_minus_data_unfolded_bayes->Write();
  x_jj_plus_data_unfolded_bayes->Write();
*/
  ...
  cout << "cross section:  "<< cross_section << "   Total events:   " << nevt_tot << "  total weights "<<nweight_total<< "   scale  "<< scale << endl;   
