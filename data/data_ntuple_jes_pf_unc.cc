...
void data_ntuple_jes_pf_unc(bool jet = false, bool sys_up = false,  const Int_t nevt_max = -1){ 
...
TH1F* test = new TH1F("test","",50,0,0.2);
  ...
  const char* jec_unc_srcnames[n_jec_unc_src] = {"AbsoluteScale","AbsoluteMPFBias", "Fragmentation", "SinglePionECAL","SinglePionHCAL", "FlavorQCD", "TimeEta", "TimePt",
                                         "RelativeJEREC1", "RelativeJEREC2", "RelativeJERHF", "RelativePtBB",
                                         "RelativePtEC1"/*new*/, "RelativePtEC2"/*new*/, "RelativePtHF"/*new*/, "RelativeFSR",
                                         "RelativeStatEC2", "RelativeStatHF", "RelativeStatFSR", "PileUpMuZero"};
...
//   vdirs->push_back("/mnt/hadoop/cms/store/user/lhuertas/CMSTOTEM_2012/MergedNtuples/reReco/198902-8369_8371-V00-02-00/Jets1/");
 //  vdirs->push_back("/mnt/hadoop/cms/store/user/lhuertas/CMSTOTEM_2012/MergedNtuples/reReco/198902-8369_8371-V00-02-00/Jets2/");
  // vdirs->push_back("/mnt/hadoop/cms/store/user/lhuertas/CMSTOTEM_2012/MergedNtuples/reReco/198903-8372-V00-02-00/Jets1/");
   //vdirs->push_back("/mnt/hadoop/cms/store/user/lhuertas/CMSTOTEM_2012/MergedNtuples/reReco/198903-8372-V00-02-00/Jets2/");
...
//     tree->SetBranchAddress("particleFlow",&pFlow_coll);
    ...
    for(int i_evt = 0; i_evt < nev && i_tot < nevt_max_corr; ++i_evt , ++i_tot){
      ...
      for(; it_hlt != it_hlt_end; ++it_hlt){
         string const& hltName = it_hlt->first;//cout<<"hlt: " <<hltName<<endl;
...
//        ofs << i_evt << it_trk->Eta() << it_trk->Phi() << endl;
     ...
     // func->SetParameter(0,5.525);
      ...
      for(vector<MyPFJet>::iterator it_jet = pfJet_coll->begin(); it_jet != pfJet_coll->end() ; ++it_jet,++idx_jet){
         ...
         CoorFactor = JetL2RelativeL3AbsoluteL2L3Residual.Pt()/UnCorrectedJet.Pt(); 

         /*Jet1_corr_px = Jet1L2RelativeL3AbsoluteL2L3Residual.Px(); 
         Jet1_corr_py = Jet1L2RelativeL3AbsoluteL2L3Residual.Py(); 
         Jet1_corr_pz = Jet1L2RelativeL3AbsoluteL2L3Residual.Pz(); 
         Jet1_corr_energy = Jet1L2RelativeL3AbsoluteL2L3Residual.E(); 
         Jet1_corr_pt = Jet1L2RelativeL3AbsoluteL2L3Residual.Pt(); 
         Jet1_corr_eta = Jet1L2RelativeL3AbsoluteL2L3Residual.Eta(); 
         Jet1_corr_phi = Jet1L2RelativeL3AbsoluteL2L3Residual.Phi();*/
	 ...
	 Jet1_phi = leadingJet.Phi(); 
         ...
         if(jet==true) Jet1_pt = (sys_up == true) ? Jet1_pt*(1. + sqrt(sum2_up)) : Jet1_pt*(1. - sqrt(sum2_dw));
         else Jet1_pt = Jet1_pt;
	 ...
	 Jet2_phi = secondJet.Phi(); 
         ...
         if(verbose) std::cout << "second jet pT, uncertainties total, individual sources up/dw: "
                      ...
                      << ", " << sqrt(sum2_up) << ", " << sqrt(sum2_dw) << std::endl;
         if(jet==true) Jet2_pt = (sys_up == true) ? Jet2_pt*(1. + sqrt(sum2_up)) : Jet2_pt*(1. - sqrt(sum2_dw));
         else Jet2_pt = Jet2_pt;
